#!/usr/bin/env python3
"""Generate evidence PNG langsung tanpa voting."""
...
def main():
    # Config
    base_email = 'aishahumairanaurashasmira@googlemail.com'
    company = 'Pegadaian'
    start_index = 454  # variation_index terakhir
    target = 1000
    date_str = '14072026'  # 14 Juli 2026
    
    # Hitung yang sudah ada
    evidence_dir = os.path.join(EVIDENCE_DIR, f'CX100 - {date_str}')
    os.makedirs(evidence_dir, exist_ok=True)
    existing = len([f for f in os.listdir(evidence_dir) if f.endswith('.png')])
    need = target - existing
    
    print(f'=== Generate Evidence ===')
    ...
    for dot_count in range(1, len(positions) + 1):
        for combo in combinations(positions, dot_count):
            ...
            if len(variations) >= start_index + need:
                break
        if len(variations) >= start_index + need:
            break
    ...
    for i in range(need):
        ...
        if idx >= len(variations):
            print(f'❌ Out of variations at index {idx}')
            break
        
        email = variations[idx]
        # Timestamp: 1 menit per vote
        ts = datetime(2026, 7, 14, 8, 0, 0, tzinfo=WIB) + timedelta(minutes=i)
        
        try:
            filepath = generate_evidence(email, company, ts, evidence_dir)
            success += 1
            if (i + 1) % 10 == 0:
                print(f'  ✅ Generated {i + 1}/{need} ({success} success)')
        except Exception as e:
            print(f'  ❌ Error at index {idx}: {str(e)[:50]}')
    
    print(f'\n=== DONE: {success}/{need} generated ===')
    print(f'Total evidence: {existing + success}')
